...
import com.dylibso.chicory.dwarf.DebugInfo.Language;
...
public final class RustParser {
    ...
    public static DebugInfo parse(byte[] wasm) throws ParserException {
    ...
    public static DebugInfo parse(InputStream is) throws ParserException {
        ArrayList<DebugInfo.Entry> entries = new ArrayList<>();
            ...
            for (var unit : result.units) {
                for (var file : unit.files) {
                    ...
                    for (var location : file.lines) {
                        ...
                        entries.add(new DebugInfo.Entry(path, file.file, language, address, line));
                    }
                }
            }
