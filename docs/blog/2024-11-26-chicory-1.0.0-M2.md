...
We finalized making the modules compiled at build time with the `aot-maven-plugin-experimental` completely self-contained, you can now emit a binary that runs pure Java Bytecode and only depends on the Chicroy runtime data-structures.
