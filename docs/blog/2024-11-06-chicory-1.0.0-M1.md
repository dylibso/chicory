...
But this release is somewhat special:
...
- but also, this release marks the beginning of our path towards a final **1.0 version**.

The reason why this release took some time is we wanted to get as many thing right. Let's take a look.
...
[updated instructions on the docs][docs]. For instance, this is how you used to instantiate a Module:

```java
Module m = Module.builder("path/to/your.wasm").build();
Instance inst = m.instantiate();
```
There is now a clearer boundary between `Parser`, `Module`s and `Instance`s:

```java
Module m = Parser.parse("path/to/your.wasm");
...
var inst = store.instantiate("logger", Parser.parse(new File("./logger.wasm")));
```
...
You can instantiate a module by explicitly providing a `MachineFactory`. The default `Machine` implementation is the
`InterpreterMachine`. You can opt in to the AoT mode by writing: 

```java

Module m = Parser.parse("path/to/your.wasm");
Instance inst = Instance.builder(m)
...
This mode of execution requires to configure a Maven plug-in. [An example can be found in the Chicory
codebase][chicory-wabt]. The [`wabt`][wabt] suite of tools allows to manipulate Wasm bytecode 
in a number of ways. For instance, the `wat2wasm` tool takes WAT source files (written in "WebAssembly Text Format")
...
This improves the compatibility of our engine
with the output of the majority of compilers targeting `wasip1`; at this point, the most used functions
should be all supported. Refer to the [WASI docs][wasi] for details.
